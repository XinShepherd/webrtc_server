const SkyRTC = function() {
  ...
  skyrtc.prototype.sendOffers = function() {
    ...
    for (i = 0, m = this.connections.length; i < m; i++) {
      pc = this.peerConnections[this.connections[i].socketId]
      pc.createOffer(pcCreateOfferCbGen(pc, this.connections[i].socketId), pcCreateOfferErrorCb)
    }
  }
  ...
  skyrtc.prototype.createPeerConnections = function() {
    var i, m
    for (i = 0, m = this.connections.length; i < m; i++) {
      this.createPeerConnection(this.connections[i].socketId)
    }
  }
  ...
  skyrtc.prototype.addDataChannel = function(socketId, channel) {
    ...
    channel.onmessage = function(message) {
      ...
      } else if (json.type === 'send2server') {
        that.emit('data_channel_message_drag_server', channel, socketId, json)
      } else if (json.type === 'send2client') {
        that.emit('data_channel_message_drag_client', channel, socketId, json)
      } else {
